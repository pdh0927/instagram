...
class _MyAppState extends State<MyApp> {
  ...
  Widget build(BuildContext context) {
    return  Scaffold(
      ...
      body: [Home(content: content, addData: addData,), Shop()][tab],  // 2. state에 따라 tab이 어떻게 보일지
      // body: [MediaQuery.of(context).size.width>600 ? HomeLarge() : Home(content: content, addData: addData,), Shop()][tab],  // 2. state에 따라 tab이 어떻게 보일지
...
class _HomeState extends State<Home> {
  ...
  Widget build(BuildContext context) {
    ...
    MediaQuery.of(context).size.width;  // 현재 폰 화면 가로사이즈(LP 단위(1cm = about 38 LP))
    MediaQuery.of(context).size.height;  // 현재 폰 화면 세로사이즈
    MediaQuery.of(context).padding.top;  // 현재 폰 상태바 윗 여백
    MediaQuery.of(context).devicePixelRatio;  // 해상도(1LP에 몇 pixel인지)
    MediaQuery.of(context).highContrast;  // 고대비옵션 켜져있는지
    MediaQuery.of(context).textScaleFactor;  // 폰트사이즈 얼마나 키우고 폰 쓰는지
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(widget.content[i]['content'], style: TextStyle(  // 반응형
